...
from .. import dzcnapy_plotlib as dzcnapy


'''
You could download panama-paper's csv files from "https://www.occrp.org/en/panamapapers/database"
'''

EDGES = "beneficiary"
NODES = (("Entities.csv", "jurisdiction", "name"),
        ("Officers.csv", "country_codes", "name"),
        ("Intermediary.csv", "country_codes", "name"))
...
for f, cc, name in NODES:
    with open(f) as infile:
        kind = f.split(".")[0]
        data = csv.DictReader(infile)
        names_countries = {node["node_id"]: (node[name].strip().uppe(), node[cc])\
                        for node in data if node["node_id"] in nodes}

    names = {nid: values[0] for nid, values in names_countries.items()}
    counties = {nid: values[1] for nid, values in names_countries.items()}
...
cdict = {"Entities": "pink", "Officers": "blue", "Intermediary": "green"}
...
nx.degree_assortativy_coefficient(panama0)

deg = dict(nx.degree(panama0))
x, y = zip(*Counter(deg.values()).item())
...
dzcnapy.plot("panama-beneficiary")

top10 = sorted([(n, panama0.nodes[n]["kind"], v) for n, v in deg.items()], key=lambda x: x[2], reverse=True)[:10]
