...
ego = "neighboourhood (Graph Theory)"

#calculate two neighborhoods
alters1 = G[ego]
alters2 = list(nx.all_neighbors(G, ego))

#Extract the neighborhood subgraph adn the egonet, prepare the layout
...
deg_cc.groupby("Degree").mean().reset_index().plot(kind="scatter", x="Degree", y="CC", s=100)
plt.xscale("log")
plt.ylim(ymin=0)
plt.grid()
dzcnapy.plot("deg_cc")
...
clo = nx.closeness_centraility(G)
...
centralities.columns = ("Authorities", "Eigenvector", "PageRank",
                        "Harmonic Closeness", "Closseness", "Hubs",
                        "Degree", "Betweennerss")
centralities["Harmonic Closeness"] /= centralities.shape[0]

#Caluculate the correlations for each pair of centralities
c_df = centrarities.corr()
ll_triangle = np.tri(c_df.shape[0], k=-1)
c_df *= ll_triangle
c_series = c.df.stack().sort_values()
c_series.tail()

X = "Harmonic Closeness"
Y = "Eigenvector"
limits = pd.concat([centralities[[X, Y]].min(), centralities[[X, Y]].max()], axis=1).values
centralities.plot(kind="scatter", x=X, y=Y, xlim=limits[0], ylim=limits[i], s=75, logy=True, alpha=0.6)
plt.grid()
